...
class MessagesBus {
  ...
  constructor(opts: ClientOpts[]) {
    ...
    for (let i = 0; i < opts.length; i++) {
      ...
      this.sub
        .getServer(i)
        .on("messageBuffer", (chBuff: Buffer, data: Buffer) => {
          const channel = chBuff.toString();
          if (!e[channel]) return;
          console.log(
            `${clc.bold(clc.green("REC  " + i))} ${clc.yellow(
              channel
            )} ${JSON.stringify(msgpack.decode(data))}`
          );
  ...
  join(channel: string, cb: EventCallback): void {
    ...
    if (c.length == 0) {
      ...
      console.log(
        `${clc.bold(clc.yellow("SUB  " + i))} ${clc.yellow(channel)}`
      );
    }

    //Подписываемся локально
    if (!!~c.indexOf(cb))
      throw new Error(`Duplicate callback for "${channel}"`);
    c.push(cb);
  ...
  publish(channel: string, data: unknown): void {
    const i = this.sub.indexOf(channel);
    console.log(
      `${clc.bold(clc.blue("PUB  " + i))} ${clc.yellow(
        channel
      )} ${JSON.stringify(data)}`
    );
        ...
        Object.keys(sep).map(
          (i) =>
            new Promise((resolve, reject) => {
              this.pub
                .getServer(parseInt(i))
                .pubsub("NUMSUB", sep[i], (err, whyIAmANumber) => {
                  if (err) reject(err);
...
class DialogsConrtoller {
  ...
  async get(id: string): Promise<string[]> {
    ...
    return await new Promise<string[]>((resolve, reject) => {
      this.dcache
        .serverOf(id)
        .smembers(id, (err, item) => (err ? reject(err) : resolve(item)));
    });
  }
  ...
  async getFor(uid: string, did: string): Promise<string[]> {
    ...
    if (!~members.indexOf(uid))
      throw new Error(`User "${uid}" doesn't have access to dialog "${did}"`);
    return members;
  }
